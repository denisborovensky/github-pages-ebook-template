...
run App
```
Теперь, чтобы запустить веб-сервер и начать обрабатывать запросы, нам достаточно выполнить в терминале команду `rackup` (rackup предоставляет универсальный интерфейс для запуска приложений Rack на поддерживаемых серверах, включая Puma, и поставляется в виде отдельного джема):
```bash
rackup
```

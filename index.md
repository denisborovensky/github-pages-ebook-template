...
Чтобы продемонстрировать, как работает **lucid_http**, я создал небольшой скрипт, код которого размещен в файле [appendix_lucid_http_app.ru](assets/scripts/book/appendix_lucid_http_app.ru). В [приложении](#prilozhenie-dzhem-lucid-http) я подробно расскажу, как его запустить.  
...
По умолчанию базовый **URL**-адрес, на который **lucid_http** отправляет запросы, — [http://localhost:9292](http://localhost:9292). Обратите внимание, что в конце адреса отсутствует завершающая косая черта, а значит нам необходимо включить ее в путь, по которому мы хотим послать запрос (в нашем случае **/hello**).  
...
rackup
```
Если после этого перейти по ссылке [http://localhost:9292/hello](http://localhost:9292/hello), то мы увидим наше первое сообщение. Ура!  
...
run App
```
Если мы снова перейдем по ссылке [http://localhost:9292/hello](http://localhost:9292/hello), то увидим, что ... ничего не изменилось! Почему? Все потому, что на сервере до сих пор работает наш исходный скрипт.  
...
rerun rackup
```
Перейдем на [http://localhost:9292/hello](http://localhost:9292/hello) и убедимся, что вывод обновляется без перезапуска приложения:  
...
run App
```
Если теперь мы перейдем на [http://localhost:9292/hello/Roda](http://localhost:9292/hello/Roda), то увидим, что строка `Roda` берется из пути и отображается на странице.  
...
#### Сопоставители классов  

#### Логические сопоставители  

#### Сопоставители регулярных выражений  

#### Сопоставители массивов  

#### Сопоставители хэшей  

#### Сопоставители символов  

#### Сопоставители процедур  

#### Сопоставители для чего угодно  

### Прочие методы RodaRequest  
...
### Область видимости блока route  

### Класс Roda_  

#### app, приложение rack  

#### freeze, для предотвращения неожиданных изменений  

#### opts, параметры класса и плагина  

#### plugin, для загрузки плагинов  

#### route, для создания блока маршрута  
...
# => "<h1>Hello World!<h1>"
```
По умолчанию **lucid_http** отправляет запрос на URL-адрес [http://localhost:9292](http://localhost:9292), в чем мы можем убедиться, вызвав метод `target_url`:
```ruby

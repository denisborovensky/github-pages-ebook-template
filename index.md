...
На этом история не заканчивается. Если мы внимательно рассмотрим наш предыдущий пример, то убедимся, что у нас имеется два блока кода: `route`, принимающий запрос, и `r.on`, генерирующий контент. Давайте посместим операторы отладки `p` внутрь каждого из них:
```ruby
  require "roda"
...
```
Если же мы перейдем на [http://localhost:9292/hello](http://localhost:9292/hello), то будут напечатаны уже обе строки:
```shell
"ROUTE block"
"HELLO block"
